    ...
    const getCheckedValues = (form, name) => {
        return [...form.querySelectorAll(`input[name="${name}"]:checked`)]
            .map((input) => {
                ...
                const inlineText = label?.querySelector('.inline-input')?.value.trim();
                return inlineText ? `${input.value}: ${inlineText}` : input.value;
            })
            .join(', ');
    };
        ...
        contactForm.addEventListener('submit', async (e) => {
            ...
            try {
                await submitLead({
                    ...
                    contactTime: getCheckedValues(contactForm, 'contact_time'),
                    employeeCount: '',
                    managerName: '',
                    message: '',
