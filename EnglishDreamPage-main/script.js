document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            if (targetElement) {
                // If targeting curriculum, show it first
                if (targetId === '#curriculum') {
                    targetElement.classList.add('show');
                    targetElement.querySelectorAll('.fade-in').forEach(el => {
    ...
    // Check if initial hash is #curriculum
    if (window.location.hash === '#curriculum') {
        const curriculumSection = document.getElementById('curriculum');
        if (curriculumSection) {
            curriculumSection.classList.add('show');
            curriculumSection.querySelectorAll('.fade-in').forEach(el => {
                el.classList.add('visible');
            });
            setTimeout(() => {
                const headerOffset = header.offsetHeight + 12;
    ...
    const managementSliders = document.querySelectorAll('.management-slider');
    managementSliders.forEach(slider => {
        const cards = [...slider.querySelectorAll('.management-slider-card')];
        const dots = [...slider.querySelectorAll('.management-slider-dots button')];
        const prevButton = slider.querySelector('.management-slider-prev');
        const nextButton = slider.querySelector('.management-slider-next');
        let activeIndex = 0;
        let autoplayId;
    ...
    const boosterSliders = document.querySelectorAll('.booster-slider');
    boosterSliders.forEach(slider => {
        const cards = [...slider.querySelectorAll('.booster-slider-card')];
        const dots = [...slider.querySelectorAll('.booster-slider-dots button')];
        const prevButton = slider.querySelector('.booster-slider-prev');
        const nextButton = slider.querySelector('.booster-slider-next');
        let activeIndex = 0;
        let autoplayId;
